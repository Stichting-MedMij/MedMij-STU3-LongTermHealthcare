...
| Contacttype | Item | NL-CM:15.1.2 | Thuis (code 'HH' in codesysteem 'ActCode') | a | Type contact | Als de (gecodeerde) waarde van dit data-item gelijk is aan de NullFlavor _OTH_, wordt de specifieke omschrijving via het FHIR-element `Encounter.type(.text)` uitgewisseld. | M |
